...
int main() {
    ...
    std::string query = "Rent a girlfriend";  // Arama yapmak istediğiniz anime ismi
        ...
        for (const auto& anime : results) {
            ...
            if (episodes.empty()) {
                ...
                if (!full_watch_url.empty()) {
                    ...
                    std::string generatedURL = fetchData.watch_url(returnedSlug, seasonStr, full_watch_url);
