...
int main() {
    ...
    if (results.empty()) {
        ...
        for (const auto& anime : results) {
            ...
            std::string test_slug = "kimi-no-suizou-wo-tabetai";
            std::string test_season = "1";
            std::string test_video = "1";
                
            std::string full_watch_url = fetchData.watch_url(test_slug, test_season, test_video);
            std::cout << "Generated watch URL: " << full_watch_url << std::endl;
        }
    }
