...
std::vector<std::map<std::string, std::string>> FetchData::fetch_anime_search_data_anmcx(const std::string& query) {
    std::string modified_query = query;
    std::replace(modified_query.begin(), modified_query.end(), ' ', '-');
    std::string search_url = anmcx_base_url + "secure/search/" + modified_query + "?type=&limit=20";
...
std::vector<int> FetchData::fetch_anime_seasons_anmcx(const std::string& selected_id) {
    std::string url = anmcx_base_url + "secure/related-videos?episode=1&season=1&titleId=" + selected_id + "&videoId=" + selected_id;
...
std::vector<std::map<std::string, std::string>> FetchData::fetch_anime_episodes_anmcx(const std::string& selected_id) {
    std::vector<int> seasons = fetch_anime_seasons_anmcx(selected_id);
    std::vector<std::map<std::string, std::string>> episodes;
    std::set<std::string> seen_episode_names;

    for (int season_index : seasons) {
        std::string url = anmcx_base_url + "secure/related-videos?episode=1&season=" + std::to_string(season_index + 1) + "&titleId=" + selected_id + "&videoId=" + selected_id;
        json data = _get_json(url);
        if (!data.empty() && data.contains("videos")) {
...
std::vector<std::map<std::string, std::string>> FetchData::fetch_anime_watch_api_url(const std::string& base_url, const std::string& url) {
...
std::string FetchData::fetch_anime_watch_api_url_movie_anmcx(const std::string& selected_id) {
