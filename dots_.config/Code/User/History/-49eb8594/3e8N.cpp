...
std::vector<std::string> select_source_options = {
    "AnimeciX", "OpenAnime"
};
...
std::vector<std::map<std::string, std::string>> selectSource() 
{
    std::string source;
    source = getInputFromRofi("Kaynak Seç", {select_source_options});
    
    if (source == "AnimeciX") {
        base_url = "anm.cx";
    } else if (source == "OpenAnime") {
        base_url = "openani.me";
    }
}
...
std::vector<std::map<std::string, std::string>> queryLoop() 
{
    ...
    while (true) 
    {
        ...
        results = fetchdata.fetch_anime_search_data_anmcx(query);
...
int main(int argc, char* argv[]) {
    ...
    anime_episodes = fetchdata.fetch_anime_episodes_anmcx(selected_id);
    ...
    if (is_movie) 
    {
        movie_url = fetchdata.fetch_anime_watch_api_url_movie_anmcx(selected_id);
    }
    ...
    while (true) 
    {
        ...
        else if (main_menu_choice == "İzle") 
        {
			...
			if (!is_movie) 
            {
            	...
            	std::vector<std::map<std::string, std::string>> watch_url = fetchdata.fetch_anime_watch_api_url(base_url, episode_url);
                if (!watch_url.empty()) 
                {
                    // URL'yi al	
