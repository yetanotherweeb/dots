...
std::string FetchData::make_request(const std::string& url) {
    ...
    // Timeout ve verbose mode ekle
    curl_easy_setopt(curl, CURLOPT_TIMEOUT, 30L);  // Timeout 30 saniye
    curl_easy_setopt(curl, CURLOPT_VERBOSE, 1L);  // Debugging için verbose aktif et
...
std::string FetchData::watch_url(const std::string& slug, const std::string& season_number, const std::string& video) {
    std::string watching_url;
    watching_url = player + "/animes/" + slug + "/" + season_number + "/" + video;
