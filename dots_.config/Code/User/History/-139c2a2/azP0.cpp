...
int main() {
    ...
    while (true) 
    {
        ...
        {
            main_menu_options = {
			    "İzle", "Sonraki Bölüm", "Önceki Bölüm", "Bölüm Seç", "Anime Ara", "Çık", std::get<0>(anime_episodes[selected_episode_index])
		    };
        }

        else if (is_movie) {
            main_menu_options = {
                "Film İzle", "Anime Ara", "Çık"
            };
        }
        ...
        else if (main_menu_choice == "Anime Ara") 
        {
            ...
            // Seçilen animeye ait slug
            slug = selected_anime.at("slug");
        
            // API'den sezon ve bölüm bilgilerini al
            anime_seasons = fetchdata.fetch_anime_seasons_openani(slug);
            anime_episodes = fetchdata.fetch_anime_season_episodes(slug);
            ...
            // Her sezon ve bölüm için api_url ve ardından watch_url oluştur
            for (int season = 1; season <= total_seasons; ++season) {
                // O sezonun maksimum bölüm sayısını bul
                int max_episode = 0;
                for (const auto& ep : anime_episodes) {
                    if (std::get<2>(ep) == season) {
                        max_episode = std::max(max_episode, std::get<1>(ep));
                    }
                }
            
                for (int episode = 1; episode <= max_episode; ++episode) {
                    std::string api_url = fetchdata.fetch_anime_episode_watch_api_url(slug, episode, season);
                    all_episode_urls.emplace_back(api_url, episode, season);
                
                    std::string watch = fetchdata.watch_url(slug, std::to_string(season), api_url);
                    if (!watch.empty()) {
                        all_watch_urls.emplace_back(watch, episode, season);
                    } else {
                        std::cerr << "URL alınamadı: Sezon " << season << ", Bölüm " << episode << "\n";
                    }
                }
            }
        }
        ...
        else if (main_menu_choice == "İzle") 
        {
            ...
            if (!all_episode_urls.empty()) 
            {
                ...
                std::string command = "mpv --fullscreen \"" + std::get<0>(all_watch_urls[selected_episode_index]) + "\" > /dev/null 2>&1";
