...
fn_select() {
    animation_items=$(hygo workflow list 2>/dev/null)
    ...
    selected=$(printf "%s" "$animation_items" | rofi -dmenu -i -select "$current" \
        -p "Select workflow" \
    ...
    case "$selected" in
    Default) hygo workflow set default ;;
    *) hygo workflow set "$selected" ;;
    esac
