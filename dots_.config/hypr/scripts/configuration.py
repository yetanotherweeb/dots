...
def get_var(theme_var: str, file: Optional[str] = None) -> Optional[str]:
    ...
    default_configs = [
        Path(xdg_data_home) / "hygo/hygo.conf",
        Path(xdg_data_home) / "hygo/hyprland.conf",
        Path("/usr/local/share/hygo/hygo.conf"),
        Path("/usr/local/share/hygo/hyprland.conf"),
        Path("/usr/share/hygo/hygo.conf"),
