...
image_dirs=()
hygo_distro_logo=${iconDir}/Wallbash-Icon/distro/$LOGO
...
logo) # eats around 13 ms
  random() {
    (
      ...
      if [ -n "${HYDE_THEME}" ] && [ -d "${confDir}/hygo/themes/${HYDE_THEME}/logo" ]; then
        image_dirs+=("${confDir}/hygo/themes/${HYDE_THEME}/logo")
      fi
      [ -d "$HYDE_CACHE_HOME" ] && image_dirs+=("$HYDE_CACHE_HOME")
      [ -f "$hygo_distro_logo" ] && echo "${hygo_distro_logo}"
...
options:
  ...
  --theme   Display a logo inside the hygo theme directory
