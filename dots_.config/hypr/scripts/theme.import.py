...
REPO_URL = "https://github.com/HyDE-Project/hygo-gallery.git"
CLONE_DIR = os.path.join(
    os.getenv("XDG_CACHE_HOME", os.path.expanduser("~/.cache")),
    "hygo/gallery-database",
)
JSON_DATA = None
...
def fetch_data():
    global JSON_DATA
    json_file_path = os.path.join(CLONE_DIR, "hygo-themes.json")
...
def fzf_menu():
    try:
        ...
        subprocess.run(["hygo-shell", "reload"], check=True)
...
def fetch_all_themes():
    ...
    themes_dir = os.path.join(config_home, "hygo/themes")
...
def main():
    parser = argparse.ArgumentParser(
        description="Imports themes from hygo-gallery repository",
        epilog="Env:\n"
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="THEME",
        help="Fetch and update a specific theme by name (`all` to fetch all themes located in 'xdg_config/hygo/themes')",
    )
