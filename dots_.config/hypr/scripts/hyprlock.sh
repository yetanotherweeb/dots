...
# shellcheck source=$HOME/.local/bin/hygo-shell
# shellcheck disable=SC1091
if ! source "$(which hygo-shell)"; then
    echo "[wallbash] code :: Error: hygo-shell not found."
    echo "[wallbash] code :: Is HyDE installed?"
    exit 1
fi
scrDir=${scrDir:-$HOME/.local/lib/hygo}
confDir="${confDir:-$XDG_CONFIG_HOME}"
cacheDir="${HYDE_CACHE_HOME:-"${XDG_CACHE_HOME}/hygo"}"
...
fn_profile() {
    ...
    if [ -f "$HOME/.face.icon" ]; then
        ...
        cp "$XDG_DATA_HOME/icons/Wallbash-Icon/hygo.png" "${profilePath}.png"
    fi
    return 0
}
...
fn_mpris() {
    ...
    if [[ "${player_status}" == "Playing" ]]; then
        ...
        if [ -f "$HOME/.face.icon" ]; then
            ...
            if ! cmp -s "$XDG_DATA_HOME/icons/Wallbash-Icon/hygo.png" "${THUMB}.png"; then
                cp "$XDG_DATA_HOME/icons/Wallbash-Icon/hygo.png" "${THUMB}.png"
                pkill -USR2 hyprlock >/dev/null 2>&1 # updates the mpris thumbnail
            fi
        fi
        exit 1
...
generate_conf() {
    ...
    local hygo_hyprlock_conf=${SHARE_DIR:-$XDG_DATA_HOME}/hygo/hyprlock.conf
...
#*│ \$XDG_CONFIG_HOME/hypr/hygo.conf,                                          │
