...
Modify '$XDG_CONFIG_HOME/hygo/config.toml'  to set options.
...
fn_status() {
    ...
    "Full")
        ...
        ;;
    *)
        if [[ ! -f "/tmp/hygo.battery.notify.status.fallback.$battery_status-$$" ]]; then
            echo "Status: '==>> \"${battery_status}\" <<==' Script on Fallback mode,Unknown power supply status.Please copy this line and raise an issue to the Github Repo.Also run 'ls /tmp/hygo.battery.notify' to see the list of lock files.*"
            touch "/tmp/hygo.battery.notify.status.fallback.$battery_status-$$"
        fi
        fn_percentage
        ;;
    esac
